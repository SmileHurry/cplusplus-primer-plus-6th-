...
{
protected:
	enum{other,alto,contralto,sorpano,bass,baritone,tenor};
	enum{Vtypes=7};
	void Data() const;
	void Get();
private:
...
public:
	SingingWaiter() {}
	SingingWaiter(const std::string & s, long n, int p = 0, int v = other) : Worker(s, n), Waiter(s, n, p), Singer(s, n, v) {}//重点:显式地调用虚基类的构造函数
